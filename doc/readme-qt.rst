...
Alternatively, install Qt Creator and open the `TREAU-qt.pro` file.

An executable named `TREAU-qt` will be built.
...
A warning for people using the *static binary* version of TREAU on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of TREAU is linked against libdb5.3.
...
installed causes TREAU-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
