...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #TREAUTEST2\r");
            Send(hSocket, "WHO #TREAUTEST2\r");
        } else {
            // randomly join #TREAU00-#TREAU05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #TREAU%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #TREAU%02d\r", channel_number).c_str());
        }
